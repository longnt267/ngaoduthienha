    ...
    <form class="ct-pd row" action="{{ route('tour.store') }}" method="post" enctype="multipart/form-data">
        ...
        <div class="col-sm-6" style="border-right: 1px dashed #88888840;">
            ...
            <div class="form-group">
                ...
                <input type="number" min="1000" class="form-control @error('price') is-invalid @enderror" id="price" name="price"
            ...
            <div class="form-group">
                ...
                <select class="form-control @error('destination_id') is-invalid @enderror" id="destination-id"
                    ...
                    @foreach ($destination as $item)
                        <option value="{{ $item->id }}" {{ old('destination_id') == $item->id ? 'selected' : '' }}>{{ $item->title }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control @error('type_tour_id') is-invalid @enderror" id="type-tour-id"
                    ...
                    @foreach ($typeTour as $item)
                        <option value="{{ $item->id }}" {{ old('type_tour_id') == $item->id ? 'selected' : '' }}>{{ $item->title }}</option>
                    @endforeach
                </select>
                @error('type_tour_id')
                    <span class="invalid-feedback" role="alert">
            ...
            <div class="form-group">
                ...
                <input type="number" class="form-control @error('duration') is-invalid @enderror" id="duration"
                    name="duration" placeholder="Duration" min="1" max="10" value="{{ old('duration') }}">
                @error('duration')
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                    </span>
            ...
            <div class="form-group">
                <label for="max_people">Max people <strong>*</strong></label>
                <input type="number" class="form-control @error('max_people') is-invalid @enderror" id="max_people"
                    name="max_people" placeholder="Max people" min="1" max="50" value="{{ old('max_people') }}">
                @error('max_people')
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                    </span>
            ...
            <div class="form-group">
                <label for="possible_date">Possible dates<strong>*</strong></label>
                <input class="form-control @error('possible_date') is-invalid @enderror" id="possible_date"
                    name="possible_date" placeholder="Possible dates" value="{{ old('possible_date') }}">
                @error('possible_date')
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                    </span>
        ...
        <div class="col-sm-6">
            ...
            <div class="form-group">
                ...
                <input type="text" name="map" id="map" class="form-control" placeholder="Map" value="{{ old('map') }}">
            </div>

            <div class="form-group">
                <label for="image-360">Embed link image 360</label>
                <input type="text" name="image_360" id="image-360" class="form-control" placeholder="Image 360" value="{{ old('image_360') }}">
            </div>
        ...
        <div class="col-sm-12">
            ...
            <div class="tab-content" id="nav-tabContent">
                <div class="tab-pane fade show active " id="nav-overview" role="tabpanel" aria-labelledby="nav-overview-tab">
                    <textarea name="overview" id="overview" rows="10" cols="80">{{ old('overview') }}</textarea>
                </div>
                <div class="tab-pane fade" id="nav-included" role="tabpanel" aria-labelledby="nav-included-tab">
                    <textarea name="included" id="included" rows="10" cols="80">{{ old('included') }}</textarea>
                </div>
                <div class="tab-pane fade" id="nav-departure" role="tabpanel" aria-labelledby="nav-departure-tab">
                    <textarea name="departure" id="departure" rows="10" cols="80">{{ old('departure') }}</textarea>
                </div>
            </div>  
        </div>
        ...
        <div class="col-sm-12">
            <div class="form-group" style="margin-top: 20px;">
                ...
                <textarea name="addtional_info" id="addtional_info" rows="10" cols="80">{{ old('addtional_info') }}</textarea>
            </div>
        </div>
